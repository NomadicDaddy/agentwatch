...
Sixteen canonical Agents are recognized: claude, codex, opencode, kilo, cursor, windsurf,
antigravity, gemini, grok, kiro, copilot, zcode, agents, cline, devin, and t3code. Five
agentwatch-only inspection surfaces are also recognized:
windsurf-next, devin-next, pi, mcp, skills, plus the special `custom` Agent that is created on
the fly for directories the user passes via `--path`.
...
twenty-two Agents, eight Artifact Types, six Finding Groups, twelve Rules, twelve
